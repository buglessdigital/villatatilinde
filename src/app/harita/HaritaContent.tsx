...
function HaritaInner() {
    ...
    useEffect(() => {
        async function fetchVillas() {
            ...
            const { data } = await supabase
                .from("villas")
                .select("id, slug, name, location_label, cover_image_url, max_guests, bedrooms, bathrooms, min_price, max_price, avg_rating, position_lat, position_lng")
            ...
            if (data) {
                setAllVillas(data.map((v: any) => ({
                    id: v.id,
                    slug: v.slug,
                    name: v.name,
                    location_label: v.location_label || "",
                    cover_image_url: v.cover_image_url || "/images/sailing2.png",
    ...
    const filteredVillas = useMemo(() => {
        return allVillas.filter((v) => {
            if (people > 0 && v.max_guests < people) return false;
            return true;
        });
    }, [allVillas, people]);
    ...
    const buildMarkerContent = useCallback((villa: Villa) => {
        ...
        const min = villa.min_price > 0 ? `₺${villa.min_price.toLocaleString("tr-TR")}` : "";
        const max = villa.max_price > 0 ? `₺${villa.max_price.toLocaleString("tr-TR")}` : "";
        content.textContent = min && max ? `${min} - ${max}` : min || villa.name;
        return content;
    }, []);
    ...
    useEffect(() => {
        if (googleMapRef.current && mapLoaded && !dataLoading) {
            setMarkers(googleMapRef.current, filteredVillas);
        }
    }, [filteredVillas, mapLoaded, dataLoading, setMarkers]);
                ...
                <div className={styles.villaPreview}>
                    <Link href={`/tatilvillasi/${selectedVilla.slug}`} className={styles.villaPreviewLink}>
                        <div className={styles.villaPreviewInner}>
                            <img
                                src={selectedVilla.cover_image_url}
                            ...
                            <div className={styles.villaPreviewInfo}>
                                ...
                                <div className={styles.villaPreviewMeta}>
                                    ...
                                    />
                                    {selectedVilla.max_guests} Kişilik
                                </div>
                                {selectedVilla.avg_rating > 0 && (
                                    <div className={styles.villaPreviewScore}>
                                        <img src="/images/star.svg" alt="" style={{ height: 11 }} />
                                        <span style={{ marginLeft: 6 }}>{selectedVilla.avg_rating.toFixed(1)}</span>
                                    </div>
                                )}
                                <div className={styles.villaPreviewPrice}>
                                    <div>Fiyat Aralığı</div>
                                    <div style={{ marginTop: 2, fontWeight: 500 }}>
                                        {selectedVilla.min_price > 0 ? `₺${selectedVilla.min_price.toLocaleString("tr-TR")}` : ""}
                                        {selectedVilla.max_price > selectedVilla.min_price ? ` - ₺${selectedVilla.max_price.toLocaleString("tr-TR")}` : ""}
                                    </div>
                                </div>
                            </div>
                        </div>
