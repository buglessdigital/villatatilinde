    ...
    async function loadBlogs() {
        try {
            const { data, error } = await supabase
                .from("blogs")
                .select("id, slug, title, subtitle, cover_image_url, is_published, published_at, created_at")
        ...
        <div>
            ...
            <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", overflow: "hidden" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
                    ...
                    <tbody>
                        {fetchError ? (
                            <tr><td colSpan={5} style={{ textAlign: "center", padding: 40, color: "#dc2626" }}>Hata oluştu: {fetchError}</td></tr>
                        ) : loading ? (
                            ...
                            <tr key={blog.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                                ...
                                <td style={{ padding: "10px 16px" }}>
                                    ...
                                    <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 2, maxWidth: 400, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{blog.subtitle}</div>
                                </td>
