...
export default function AdminDestinasyonlar() {
    ...
    const [form, setForm] = useState({ name: "", slug: "", location_label: "", description: "", image_url: "", filter_param: "", sort_order: 0, is_active: true, tags: "" });
    ...
    function startEdit(dest: DestinationRow) {
        ...
        setForm({ name: dest.name, slug: dest.slug, location_label: dest.location_label || "", description: dest.description || "", image_url: dest.image_url || "", filter_param: dest.filter_param || "", sort_order: dest.sort_order, is_active: dest.is_active, tags: (dest as unknown as { tags?: string[] }).tags ? (dest as unknown as { tags: string[] }).tags.join(", ") : "" });
    }

    function startNew() {
        setEditingId("new");
        setForm({ name: "", slug: "", location_label: "", description: "", image_url: "", filter_param: "", sort_order: destinations.length + 1, is_active: true, tags: "" });
    }
    ...
    async function handleSave() {
        ...
        const payload = { ...form, slug, tags: tagsArray };
        if (editingId === "new") {
                ...
                <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 24, marginBottom: 20 }}>
                    ...
                    <div style={{ display: "flex", gap: 16, marginBottom: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <input style={inputStyle} value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })} placeholder="Deniz, Doğa, Havuz..." />
                        </div>
