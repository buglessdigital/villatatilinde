...
export default function AdminSSS() {
    ...
    const [form, setForm] = useState({ question_tr: "", answer_tr: "", sort_order: 0, is_active: true });
    ...
    function startEdit(faq: FaqRow) {
        setEditingId(faq.id);
        setForm({ question_tr: faq.question_tr, answer_tr: faq.answer_tr, sort_order: faq.sort_order, is_active: faq.is_active });
    }

    function startNew() {
        setEditingId("new");
        setForm({ question_tr: "", answer_tr: "", sort_order: faqs.length + 1, is_active: true });
    }
            ...
            {editingId && (
                <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: 24, marginBottom: 20 }}>
                    ...
                    <div style={{ marginBottom: 12 }}>
                        ...
                        <textarea style={{ ...inputStyle, height: 100, resize: "vertical" }} value={form.answer_tr} onChange={(e) => setForm({ ...form, answer_tr: e.target.value })} placeholder="Cevap metni..." />
                    </div>
                    <div style={{ display: "flex", gap: 16, alignItems: "center", marginBottom: 12 }}>
                        ...
                        <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", marginTop: 18 }}>
                            <input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} style={{ width: 18, height: 18, accentColor: "#f59e0b" }} />
                            Aktif
                        </label>
                    </div>
                ...
                {loading ? (
                    ...
                    <div key={faq.id} style={{ background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0", padding: "16px 20px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                            <div style={{ flex: 1 }}>
                                <div style={{ fontSize: 15, fontWeight: 600, color: "#1e293b", marginBottom: 6 }}>{faq.question_tr}</div>
                                <div style={{ fontSize: 13, color: "#64748b", lineHeight: 1.6 }}>{faq.answer_tr}</div>
                            </div>
                            <div style={{ display: "flex", gap: 8, marginLeft: 16, flexShrink: 0 }}>
                                <span style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>#{faq.sort_order}</span>
                                <button onClick={() => startEdit(faq)} style={{ padding: "4px 12px", borderRadius: 6, border: "1px solid #e2e8f0", background: "#fff", color: "#3b82f6", fontSize: 12, cursor: "pointer" }}>Düzenle</button>
