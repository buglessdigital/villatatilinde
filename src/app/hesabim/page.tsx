...
const STATUS_MAP: Record<string, { label: string; bg: string; color: string }> = {
    ...
    pre_approved: { label: "Ön Onay Verildi", bg: "#dbeafe", color: "#1e40af" },
...
export default function HesabimPage() {
    ...
    const percent = (() => {
        ...
        if (hasEmail && hasName) return 100;
        let count = 0;
        if (hasEmail) count++;
        if (hasName) count++;
    ...
    const fetchUserData = useCallback(async () => {
        try {
            ...
            if (profileData) {
                ...
                if (wishError) {
                    console.error("Wishlist fetch error:", wishError);
                }

                if (wishData && wishData.length > 0) {
                    console.log("Raw Wish Data:", wishData);
                    const villas = wishData
                        .map((w: any) => {
                            if (Array.isArray(w.villas)) return w.villas[0] as WishVilla;
                            return w.villas as WishVilla;
                        })
                        .filter(Boolean);
                ...
                if (profileData.email) {
                    const { data: rezData } = await supabase
                        .from("reservations")
                        .select("id, ref_code, status, check_in_date, check_out_date, nights, total_amount, prepayment_amount, admin_notes, created_at")
                        .eq("renter_email", profileData.email)
                        .order("created_at", { ascending: false });

                    if (rezData) setMyReservations(rezData);
                }
            }
        } catch (error) {
                ...
                <div className="middletp" style={{ alignItems: "flex-start" }}>
                    ...
                    <div className="no767 hesabim-sidebar">
                        ...
                        <div style={{ borderTop: "1px solid #dfdfe3", paddingTop: "1rem", marginTop: "1rem" }}>
                            ...
                            <div className="middleft btnActive hesabim-ppTab" onClick={() => document.getElementById("rezervasyonlar")?.scrollIntoView({ behavior: "smooth", block: "start" })} style={{ cursor: "pointer" }}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 512 512" fill="none" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round" style={{ marginTop: -1, marginRight: ".6rem", flexShrink: 0 }}>
                            ...
                            <div className="middleft btnActive hesabim-ppTab" onClick={scrollToWishes} style={{ cursor: "pointer" }}>
                                ...
                                {wishesArray.length > 0 && (
                                    <span style={{ marginLeft: "auto", background: "#10b981", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: 10, padding: "1px 7px", minWidth: 18, textAlign: "center" }}>
                                        {wishesArray.length}
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="middleft btnActive hesabim-ppTab" onClick={() => router.push("/kupon-olustur")} style={{ cursor: "pointer" }}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 512 512" fill="none" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round" style={{ marginTop: -1, marginRight: ".6rem", flexShrink: 0 }}>
                    ...
                    <div className="hesabim-rightSide">
                        ...
                        <div id="rezervasyonlar" style={{
                            ...
                            <div style={{ borderTop: "1px solid #dfdfe3", padding: "12px 16px" }}>
                                {myReservations.length === 0 ? (
                                    <div style={{ padding: 12, color: "#6a6a6aaa", fontSize: 14 }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                                        {myReservations.map((rez) => {
                                            ...
                                            const villaMatch = rez.admin_notes?.match(/Villa:\s*(.+)/);
                                            const villaName = villaMatch?.[1]?.split("\n")[0]?.trim() || "-";
                                            const ciDate = new Date(rez.check_in_date);
                                            const coDate = new Date(rez.check_out_date);
                                            return (
                                                <div
                                                    key={rez.id}
                                                    style={{
                                                ...
                                                >
                                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 10 }}>
                                                        <div>
                                                            <span style={{ fontWeight: 700, fontSize: 15, color: "#1e293b" }}>{villaName}</span>
                                                            <span style={{ fontSize: 12, color: "#94a3b8", marginLeft: 8, fontFamily: "monospace" }}>#{rez.ref_code}</span>
                                                        </div>
                                                    ...
                                                    <div style={{ display: "flex", gap: 20, flexWrap: "wrap", fontSize: 13, color: "#64748b" }}>
                                                        <div>
                                                            ...
                                                            {ciDate.getDate()} {MONTH_SHORT_TR[ciDate.getMonth()]} → {coDate.getDate()} {MONTH_SHORT_TR[coDate.getMonth()]} {coDate.getFullYear()}
                                                        </div>
                                                        <div>
                                                            <span style={{ color: "#94a3b8", marginRight: 4 }}>🌙</span>
                                                            {rez.nights} Gece
                                                        ...
                                                        <div>
                                                            <span style={{ color: "#94a3b8", marginRight: 4 }}>💰</span>
                                                            <strong style={{ color: "#1e293b" }}>₺{rez.total_amount?.toLocaleString("tr-TR")}</strong>
                                                        </div>
                                                    </div>
                                                    {rez.status === "pending" && (
                                                        <div style={{ marginTop: 10, fontSize: 12, color: "#92400e", background: "#fffbeb", padding: "6px 12px", borderRadius: 8 }}>
                                                            ⏳ Talebiniz inceleniyor, en kısa sürede sizinle iletişime geçilecektir.
                                                        </div>
                                                    )}
                                                    {rez.status === "pre_approved" && (
                                                        <div style={{ marginTop: 10, fontSize: 12, color: "#1e40af", background: "#eff6ff", padding: "6px 12px", borderRadius: 8 }}>
                                                            ✅ Ön onay verildi. Ödeme bekleniyor.
                                                        </div>
                                                    )}
                                                    {rez.status === "confirmed" && (
                                                        <div style={{ marginTop: 10, fontSize: 12, color: "#166534", background: "#f0fdf4", padding: "6px 12px", borderRadius: 8 }}>
                                                            🎉 Rezervasyonunuz onaylanmıştır. İyi tatiller!
                                                        </div>
                                                    )}
                                                </div>
                                            );
                        ...
                        >
                            <div className="middleft" style={{ padding: "1rem", marginTop: ".5rem", fontSize: "1.1rem", fontWeight: 600, gap: 8 }}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 512 512" fill="#ef4444" stroke="#ef4444" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M352.92 80C288 80 256 144 256 144s-32-64-96.92-64c-52.76 0-94.54 44.14-95.08 96.81-1.1 109.33 86.73 187.08 183 252.42a16 16 0 0018 0c96.26-65.34 184.09-143.09 183-252.42-.54-52.67-42.32-96.81-95.08-96.81z" />
                                </svg>
                                Favorilerim
                                {wishesArray.length > 0 && (
                                    <span style={{ background: "#10b981", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: 10, padding: "1px 7px" }}>
                                        {wishesArray.length}
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="no768" style={{ color: "#333", marginTop: 48, marginBottom: 50 }}>
                            <div
                                onClick={() => document.getElementById("rezervasyonlar")?.scrollIntoView({ behavior: "smooth", block: "start" })}
                                className="middleft" style={{
                            ...
                            <div
                                ...
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 512 512" fill="none" stroke="currentColor" strokeWidth="32" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 8, flexShrink: 0 }}>
                                    <path d="M352.92 80C288 80 256 144 256 144s-32-64-96.92-64c-52.76 0-94.54 44.14-95.08 96.81-1.1 109.33 86.73 187.08 183 252.42a16 16 0 0018 0c96.26-65.34 184.09-143.09 183-252.42-.54-52.67-42.32-96.81-95.08-96.81z" />
                                </svg>
                                Favorilerim
                                {wishesArray.length > 0 && (
                                    <span style={{ marginLeft: 8, background: "#10b981", color: "#fff", fontSize: 11, fontWeight: 700, borderRadius: 10, padding: "1px 7px" }}>
                                        {wishesArray.length}
                                    </span>
                                )}
                                <Image src="/images/longright.png" alt="" width={38} height={20} style={{ padding: 2, marginLeft: "auto", objectFit: "contain" }} />
                            </div>
